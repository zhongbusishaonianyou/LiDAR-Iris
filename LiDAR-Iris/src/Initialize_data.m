function [Iris,gtpose_xy] = Initialize_data(data_dir,pose_dir,dim ,range)
%%
num_rings = dim(1);
num_sectors = dim(2);
%%
...
for data_idx = 1:num_data
    ...
    current_iris= LiDAR_Iris(ptcloud,num_sectors, num_rings,range); 
    
    % save SFM
    Iris{index} = current_iris;   
    index=index+1;
    % display processing
